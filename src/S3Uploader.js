...
const S3Uploader = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      await axios.post(`http://34.201.52.30:3000/buckets/${selectedBucket}/upload`, formData, {
  ...
  const handleBucketChange = async (bucketName) => {
    ...
    try {
      const response = await axios.get(`http://34.201.52.30:3000/buckets/${bucketName}`);
